...
- 2026-07-04: OpenPhish en OSINT: feeds de phishing, verificacion y contexto antes de bloquear URLs (fallback editorial: la seccion `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias tambien esta agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `OpenPhish` como tema principal inedito, sin repetir `MalwareBazaar`, `CISA KEV` ni `OpenCTI`, y con un enfoque defensivo de feeds de phishing, frescura, metadatos, privacidad, licencias, falsos positivos y decisiones proporcionales)
- 2026-07-03: MalwareBazaar en OSINT: muestras, hashes y contexto defensivo antes de bloquear (fallback editorial: la seccion `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias tambien esta agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `MalwareBazaar` como tema principal inedito, sin repetir `CISA KEV`, `OpenCTI` ni `Tranco`, y con un enfoque defensivo de triage de muestras, hashes, etiquetas, YARA, API, certificados de firma de codigo, privacidad y reduccion de falsos positivos)
- 2026-07-02: CISA KEV en OSINT: vulnerabilidades explotadas, prioridad y contexto antes de parchear (fallback editorial: la seccion `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias tambien esta agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `CISA KEV` como tema principal inedito, sin repetir `OpenCTI`, `Tranco` ni `ThreatFox`, y con un enfoque defensivo de priorizacion de vulnerabilidades explotadas, inventario, exposicion, BOD 26-04, trazabilidad y reduccion de falsos positivos)
- 2026-07-01: OpenCTI en OSINT: grafo de conocimiento, observables y contexto antes de automatizar inteligencia (fallback editorial: la seccion `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias tambien esta agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `OpenCTI` como tema principal inedito, sin repetir `Tranco`, `ThreatFox` ni `BinaryEdge`, y con un enfoque defensivo de grafo de conocimiento, observables, indicadores, conectores, STIX/TAXII, trazabilidad y reduccion de falsos positivos)
- 2026-06-30: Tranco en OSINT: rankings de dominios reproducibles sin convertir popularidad en evidencia (fallback editorial: la seccion `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias tambien esta agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `Tranco` como tema principal inedito, sin repetir `ThreatFox`, `BinaryEdge` ni `GreyHatWarfare`, y con un enfoque metodologico de rankings de dominios, reproducibilidad, muestreo responsable y limites antes de extraer conclusiones)
- 2026-06-29: ThreatFox en OSINT: IOCs de malware, contexto y caducidad antes de bloquear (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `ThreatFox` como tema principal inédito, sin repetir `BinaryEdge`, `GreyHatWarfare` ni `NASA FIRMS`, y con un enfoque defensivo de indicadores de malware, caducidad, falsos positivos y verificación responsable)
- 2026-06-28: BinaryEdge en OSINT: superficie externa, servicios expuestos y contexto antes de priorizar (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `BinaryEdge` como tema principal inédito, sin repetir `GreyHatWarfare`, `NASA FIRMS` ni `MISP`, y con un enfoque defensivo de superficie externa, banners, alcance, falsos positivos y priorización responsable)
- 2026-06-27: GreyHatWarfare en OSINT: buckets cloud expuestos, contexto y verificación responsable (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `GreyHatWarfare` como tema principal inédito, sin repetir `NASA FIRMS`, `MISP` ni `Cloudflare Radar`, y con un enfoque defensivo de almacenamiento cloud público, verificación responsable, falsos positivos y remediación)
- 2026-06-26: NASA FIRMS en OSINT: anomalías térmicas, incendios y contexto antes de atribuir causas (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `NASA FIRMS` como tema principal inédito, sin repetir `MISP`, `Cloudflare Radar` ni `crt.sh/Certificate Transparency`, y con un enfoque de verificación geoespacial, señales térmicas, cronología y límites antes de atribuir causas)
- 2026-06-25: MISP en OSINT: indicadores, taxonomías y contexto para compartir inteligencia sin crear ruido (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `MISP` como tema principal inédito, sin repetir `Cloudflare Radar`, `crt.sh/Certificate Transparency` ni `EDGAR`, y con un enfoque defensivo de intercambio estructurado, taxonomías, warninglists, distribución granular y reducción de falsos positivos)
- 2026-06-24: Cloudflare Radar en OSINT: tendencias, cortes de Internet y contexto sin confundir señal con causa (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `Cloudflare Radar` como tema principal inédito, sin repetir `crt.sh`, `EDGAR` ni `OONI Explorer`, y con un enfoque de tendencias agregadas, anomalías, normalización y corroboración de cortes)
- 2026-06-23: crt.sh en OSINT: Certificate Transparency para descubrir nombres sin confundirlos con activos (fallback editorial: la sección `Ideas (candidatas)` no contiene ninguna entrada libre `- [ ]` y el bloque de historias también está agotado como selector primario; tras revisar las 3 últimas entradas reales y el histórico completo de `blog/`, se publica `crt.sh` y `Certificate Transparency` como tema principal inédito, sin repetir `EDGAR`, `OONI Explorer` ni `Google Alerts`, y con un enfoque defensivo de inventario, cronología y validación)
- 2026-06-22: EDGAR en OSINT: filings, CIK y XBRL para investigar empresas con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el bloque de historias tambien estaba agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `EDGAR` por ser un enfoque todavia inedito como entrada principal, sin repetir `OONI Explorer`, `Google Alerts` ni `OpenOwnership`, y recuperando un angulo de filings regulatorios, cronologia documental y datos estructurados)
- 2026-06-21: OONI Explorer en OSINT: medir bloqueos, leer pruebas de red y no exagerar conclusiones (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el bloque de historias tambien estaba agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `OONI Explorer` por ser un enfoque todavia inedito como entrada principal, sin repetir `Google Alerts`, `OpenOwnership` ni `IPinfo`, y recuperando un angulo de medicion abierta, censura de red y lectura prudente de evidencia tecnica)
- 2026-06-20: Google Alerts en OSINT: monitorizacion de menciones, cambios y ruido con criterio (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el bloque de historias tambien estaba agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `Google Alerts` por ser un enfoque todavia inedito como entrada principal, sin repetir `OpenOwnership`, `IPinfo` ni `Epieos`, y desplazando el foco hacia monitorizacion ligera, diseno de consultas y control de ruido)
- 2026-06-19: OpenOwnership en OSINT: beneficiarios reales, estructuras societarias y contexto antes de atribuir control (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el bloque de historias tambien estaba agotado como selector primario; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `OpenOwnership` por ser un enfoque todavia inedito como entrada principal, sin repetir `IPinfo`, `Epieos` ni `GDELT`, y recuperando un angulo de trazabilidad societaria, titularidad real y prudencia antes de atribuir control)
- 2026-06-18: IPinfo en OSINT: geolocalizacion, ASN y senales de privacidad con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el backlog historico practico ya estaba agotado como selector principal; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica `IPinfo` por ser un tema todavia inedito como entrada principal, sin repetir `Epieos`, `GDELT` ni `ViewDNS.info`, y manteniendo el foco en enrichment de `IP`, limites de geolocalizacion y prudencia antes de atribuir)
- 2026-06-17: Epieos en OSINT: busqueda inversa por correo y telefono con contexto, limites y criterio (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el backlog historico util ya estaba agotado como tema principal en `blog/`; tras revisar los 3 ultimos posts reales y el historico completo del repo, se selecciona `Epieos` por ser un tema todavia inedito como entrada principal, sin repetir `GDELT`, `ViewDNS.info` ni `RDAP/WHOIS`, y manteniendo el foco en verificacion responsable, privacidad y limites de la busqueda inversa)
- 2026-06-16: GDELT en OSINT: cobertura mediatica global, narrativas y contexto sin confundir volumen con verdad (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el backlog de historias tambien estaba agotado como tema principal inedito; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica un tema nuevo centrado en datos, cobertura mediatica y lectura de narrativas, evitando repetir `ViewDNS.info`, `RDAP/WHOIS` y `DNSDumpster`, y recuperando un angulo mas analitico que de infraestructura pura)
- 2026-06-15: ViewDNS.info en OSINT: reverse IP, WHOIS, NS y MX con contexto antes de unir puntos (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `RDAP/WHOIS`, `DNSDumpster` y `Mastodon`, y manteniendo el foco en pivotes de infraestructura, relaciones tecnicas y prudencia antes de atribuir)
- 2026-06-14: RDAP y WHOIS en OSINT: propiedad aparente, contactos y contexto sin leer de mas (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica un puente metodologico realmente inedito como tema principal, enlazado ademas con el siguiente paso natural del post sobre `DNSDumpster`, evitando repetir `DNSDumpster`, `Mastodon` y `gowitness`, y manteniendo el foco en propiedad aparente, contexto registral y cautela antes de atribuir)
- 2026-06-13: DNSDumpster en OSINT: DNS visible, subdominios, netblocks y banners con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `Mastodon`, `gowitness` y `Netcraft Site Report`, y manteniendo el foco en descubrimiento inicial de superficie visible, lectura prudente de DNS y uso responsable de banners)
- 2026-06-12: Mastodon en OSINT: busqueda federada, hashtags y contexto sin confundir alcance con evidencia (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `gowitness`, `Netcraft Site Report` y `OpenAlex`, y manteniendo el foco en federacion, busqueda limitada por diseno y verificacion contextual antes de atribuir)
- 2026-06-11: gowitness en OSINT: capturas visuales, priorizar superficie web y mantener contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `Netcraft Site Report`, `OpenAlex` y `httpx`, y manteniendo el foco en triage visual, revision de interfaces visibles y prudencia antes de inferir demasiado a partir de una captura)
- 2026-06-10: Netcraft Site Report en OSINT: infraestructura, cronologia y contexto antes de atribuir (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `OpenAlex`, `httpx` y `WAFW00F`, y manteniendo el foco en contexto de infraestructura, cronologia visible y prudencia antes de atribuir)
- 2026-06-09: OpenAlex en OSINT: autores, instituciones, financiacion y trazabilidad academica (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `httpx`, `WAFW00F` y `Subfinder`, y desplazando el foco desde superficie web hacia investigacion academica, redes de colaboracion y contexto de financiacion visible)
- 2026-06-08: httpx en OSINT: sondear servicios web, priorizar hosts vivos y mantener contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `WAFW00F`, `Subfinder` y `FullHunt`, y manteniendo el foco en triage web, prioridad de hosts vivos y contexto antes de profundizar)
- 2026-06-07: WAFW00F en OSINT: identificar WAF, entender respuestas y no sobreinterpretar el perimetro web (fallback editorial: la seccion `Ideas (candidatas)` ya no tenia ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `Subfinder`, `FullHunt` y `urlquery`, y manteniendo el foco en fingerprinting web responsable, contexto HTTP y prudencia atributiva)
- 2026-06-06: Subfinder en OSINT: subdominios pasivos, fuentes y contexto antes de ampliar alcance (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y, tras revisar las 3 ultimas entradas reales y el historico completo de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `FullHunt`, `urlquery` y `TinEye`, y manteniendo el foco en enumeracion pasiva, cobertura de fuentes y verificacion contextual antes de ampliar alcance)
- 2026-06-05: FullHunt en OSINT: superficie externa, priorizacion de exposicion y contexto antes de escalar (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el backlog historico enlazado ya estaba cubierto como tema principal en `blog/`; tras revisar las 3 ultimas entradas reales y el historico completo del repo, se publica una herramienta realmente inedita como tema principal, evitando repetir `urlquery`, `TinEye` y `Maltiverse`, y manteniendo el foco en descubrimiento de superficie, cronologia pasiva y priorizacion responsable)
- 2026-06-04: urlquery en OSINT: analizar URLs, redirecciones y carga web con contexto de sandbox (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y una comprobacion tardia detecto que un borrador sobre `MH17` duplicaba un tema ya publicado como entrada principal; se sustituye por una herramienta realmente inedita en `blog/`, evitando repetir `TinEye`, `Maltiverse` y `crt.sh`, y manteniendo el foco en analisis de carga web, redirecciones y triage responsable)
- 2026-06-03: TinEye en OSINT: verificar imagenes, rastrear origen y poner contexto antes de concluir (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar las 3 ultimas entradas reales y el historico de `blog/`, se publica una herramienta realmente inedita como tema principal, evitando repetir `Maltiverse`, `crt.sh` y `AbuseIPDB`, y manteniendo el foco en verificacion visual, cronologia prudente y limites de atribucion)
- 2026-06-02: Maltiverse en OSINT: enriquecer IoCs, filtrar ruido y decidir con mas contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; durante la comprobacion de anti-duplicacion se detecto que `Navalny` ya existia como tema principal en `blog/`, asi que se sustituyo por una herramienta realmente inedita en el repo, evitando repetir `crt.sh`, `AbuseIPDB` y `Ahmia`, y manteniendo el foco en triage, enrichment y verificacion contextual)
- 2026-05-24: crt.sh en OSINT: CT logs, subdominios y cronologia de certificados con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y el backlog historico enlazado ya estaba cubierto como tema principal en `blog/`; tras revisar el historico real del repo, se publica una herramienta/interfaz concreta todavia inedita como tema principal, evitando repetir `AbuseIPDB`, `Ahmia` y `Favicon hashing`, y manteniendo el foco en trazabilidad temporal, descubrimiento de superficie y prudencia atributiva)
- 2026-05-23: AbuseIPDB en OSINT: reputacion de IPs, triage y contexto antes de bloquear (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; durante la validacion final se detecto que `Have I Been Pwned` ya existia como tema principal en `blog/`, asi que se sustituyo por una herramienta realmente inedita, evitando repetir `Ahmia`, `Favicon hashing` y `Historia OSINT: Skripal`, y manteniendo el foco en reputacion comunitaria, minimizacion y triage defensivo con contexto)
- 2026-05-22: Ahmia en OSINT: buscar servicios .onion publicos con contexto y filtros (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar el historico real de `blog/`, se publica una herramienta inedita como tema principal, evitando repetir `Historia OSINT: Skripal`, `Favicon hashing` y `Hunter Domain Search`, y manteniendo el foco en descubrimiento publico, filtrado de resultados y corroboracion antes de profundizar)
- 2026-05-21: Favicon hashing en OSINT: clusters visuales, infraestructura parecida y contexto antes de atribuir (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar el historico real de `blog/`, se publica una metodologia/herramienta inedita como tema principal, evitando repetir `Historia OSINT: Skripal`, `Hunter Domain Search` y `Common Crawl`, y manteniendo el foco en trazabilidad, clusters tecnicos y corroboracion antes de atribuir)
- 2026-05-20: Historia OSINT: Skripal, identidades de cobertura y el momento en que dos turistas dejaron de parecer turistas (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`, asi que se tomo la siguiente historia realmente inedita del backlog enlazado en `ideas/historias-osint.md`; se evita repetir `Hunter Domain Search`, `Common Crawl` y `bgp.tools`, se recupera la alternancia herramienta/historia y se deja constancia explicita de la excepcion aplicada)
- 2026-05-19: Hunter Domain Search en OSINT: correos profesionales, fuentes publicas y contexto antes de escribir (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y `ideas/historias-osint.md` no conservaba una historia realmente inedita como tema principal en `blog/`; tras revisar las 3 ultimas entradas reales se publica una herramienta nueva no tratada como post principal, evitando repetir `Common Crawl`, `bgp.tools` y la historia de Mariupol, y manteniendo el foco en trazabilidad, minimizacion y verificacion contextual)
- 2026-05-18: Common Crawl en OSINT: indice web historico, WARC y contexto para investigar sin ahogarte en ruido (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar el historico real de `blog/`, se elige una herramienta/metodologia inedita como tema principal, evitando repetir `bgp.tools`, la historia de Mariupol y `OpenCorporates`, y manteniendo el foco en memoria web, cronologia y trazabilidad)
- 2026-05-17: bgp.tools en OSINT: visibilidad de ASN, rutas y contexto antes de atribuir infraestructura (fallback editorial: la seccion `Ideas (candidatas)` seguia sin entradas libres; tras revisar el historico real de `blog/`, se publica una herramienta inedita como tema principal para recuperar la alternancia despues de la historia de Mariupol, evitando repetir `OpenCorporates`, `SpiderFoot` y la propia entrada historica anterior, y manteniendo el foco en routing, cobertura y cautela atributiva)
- 2026-05-16: Historia OSINT: Mariupol, el teatro, la palabra CHILDREN y la cronologia que no conviene leer deprisa (fallback editorial: `Ideas (candidatas)` seguia sin entradas libres y `ideas/historias-osint.md` ya no conservaba historias ineditas como tema principal en `blog/`; se publica una historia nueva extraida de la investigacion larga referenciada por el propio backlog, evitando repetir `OpenCorporates`, `SpiderFoot` y `Lampyre` y recuperando la alternancia entre herramienta e historia con foco en satelite, video y prudencia metodologica)
- 2026-05-15: OpenCorporates en OSINT: registros mercantiles, cargos y contexto societario (fallback editorial: la seccion `Ideas (candidatas)` seguia sin entradas libres y `ideas/historias-osint.md` no conservaba una historia realmente inedita como tema principal en `blog/`; se publica una herramienta/metodologia de verificacion societaria no tratada todavia como post principal, evitando repetir `SpiderFoot`, `Lampyre` y `Maigret`, y manteniendo foco en trazabilidad, registros oficiales y debida diligencia responsable)
- 2026-05-14: SpiderFoot en OSINT: automatizacion, modulos y correlaciones con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin entradas libres y el bloque historico marcado ya no aportaba una candidata accionable; se publica una herramienta relevante no tratada como tema principal en `blog/`, evitando repetir `Lampyre`, `Maigret` y `ZGrab2`, y manteniendo foco en automatizacion responsable, correlaciones y validacion)
...
- 2026-05-09: Wikidata en OSINT: entidades, propiedades y consultas con contexto antes de concluir (fallback editorial: la seccion `Ideas (candidatas)` seguia agotada y el backlog historico util estaba ya cubierto como tema principal; se publica una herramienta de datos estructurados no tratada en `blog/`, evitando repetir las 3 ultimas entradas y manteniendo el foco en desambiguacion, referencias y consultas reproducibles)
- 2026-05-08: Overpass Turbo en OSINT: contexto cartografico, consultas responsables y exportes utiles (fallback editorial: la seccion `Ideas (candidatas)` seguia agotada, asi que se publica una herramienta no tratada como tema principal en `blog/`, evitando repetir las 3 ultimas entradas y dejando trazado el criterio aplicado)
- 2026-05-07: ArchiveBox en OSINT: archivo web autocustodiado y evidencia reproducible sin depender de terceros (fallback editorial: la seccion `Ideas (candidatas)` seguia agotada y el backlog historico util estaba cubierto o desalineado con el estado real de `blog/`; se publica una herramienta nueva no tratada como tema principal, manteniendo foco metodologico y sin repetir las 3 ultimas entradas)
- 2026-05-06: Sentinel Hub EO Browser en OSINT: comparativas temporales, nubosidad y prudencia GEOINT (fallback editorial: `Ideas (candidatas)` seguia sin entradas libres y el backlog historico ya estaba cubierto como post principal; se publica un puente metodologico explicitamente sugerido por la entrada de Rohingya, evitando repetir las 3 ultimas entradas de infraestructura)
- 2026-05-05: IVRE en OSINT: recon activa, pasiva y vista consolidada para investigar con control (fallback editorial: `Ideas (candidatas)` seguia sin entradas libres y el backlog de historias estaba desalineado con el blog real, con casi todas las candidatas ya publicadas; se elige una herramienta nueva no tratada como post principal en `blog/`, evitando repetir las 3 ultimas entradas y dejando constancia del criterio aplicado)
- 2026-05-04: Maltego en OSINT: grafos, transforms y contexto para investigar sin confundir conexiones con evidencia (fallback editorial: la seccion `Ideas (candidatas)` seguia sin entradas libres y el bloque de historias candidatas ya estaba marcado como usado; se elige un tema no tratado como post principal en `blog/`, evitando repetir las 3 ultimas entradas y actualizando el backlog con el criterio realmente aplicado)
- 2026-05-03: RiskIQ PassiveTotal en OSINT: passive DNS, WHOIS y TLS para correlacionar infraestructura con contexto (fallback editorial: `Ideas (candidatas)` seguia vacia y el backlog historico practico tambien estaba agotado o desactualizado; se elige un tema no tratado como post principal en `blog/` y sin repetir las 3 ultimas entradas)
