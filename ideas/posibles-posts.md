...
- 2026-05-22: urlscan.io en OSINT: DOM, recursos y capturas para investigar webs con contexto (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]`; tras revisar el historico real de `blog/`, se publica una herramienta inedita como tema principal, evitando repetir `Historia OSINT: Skripal`, `Favicon hashing` y `Hunter Domain Search`, y manteniendo el foco en captura reproducible, visibilidad del escaneo y corroboracion antes de concluir)
