...
- 2026-06-04: Historia OSINT: MH17, el convoy Buk y la cronologia abierta que acabo en un tribunal (fallback editorial: la seccion `Ideas (candidatas)` seguia sin ninguna entrada libre `- [ ]` y `ideas/historias-osint.md` tampoco conservaba una historia realmente inedita como tema principal en `blog/`; tras revisar las 3 ultimas entradas reales y el historico de historias ya publicadas, se extrae una nueva candidata desde la investigacion larga referenciada por el propio backlog, se evita repetir `TinEye`, `Maltiverse` y `crt.sh`, y se recupera la alternancia herramienta/historia con foco en cronologia, geolocalizacion y corroboracion judicial)
